...
import os
import os.path
from backtester.dataSource.data_source_utils import readLocalFileFromDrive
...
class LocalStockDataSource(DataSource):
    def __init__(self,pathToHistoricalData, cachedFolderName, dataSetId, instrumentIds, startDateStr, endDateStr, event='history', adjustPrice=False, downloadId=".NS", liveUpdates=True, pad=True):
        super(LocalStockDataSource, self).__init__(cachedFolderName, dataSetId, instrumentIds, startDateStr, endDateStr)
        ...
        self._pathToHistoricalData = pathToHistoricalData
        self.event = event
    ...
    def downloadAndAdjustData(self, instrumentId, fileName):
        if not os.path.isfile(fileName):
            if not readLocalFileFromDrive(self._pathToHistoricalData, self._startDate, self._endDate, instrumentId, fileName):
...
if __name__ == "__main__":
    ...
    yds = LocalStockDataSource(cachedFolderName='yahooData/',
